...
let greaterthan2017 = false;
for (let movie of movies) {
  if (parseInt(movie.year) < 2017) {
    greaterthan2017 = true;
    break;
  } else {
    greaterthan2017 = false;
  }
}
console.log(greaterthan2017);
...
if (movies[0].title === "Hannah") {
  console.log(`Title of movie: ${Hannah}`);
} else {
  console.log("not");
}
...
for (let i = 0; i < movies.length; i++) {
  console.log(
    `numbers of actors in movie ${i + 1} is: ${movies[i].actors.length}`
  );
}
// 26. Log the number of genres in all three movies one by one
for (let i = 0; i < movies.length; i++) {
  console.log(
    `numbers of genres in movie ${i + 1} is: ${movies[i].genres.length}`
  );
}
